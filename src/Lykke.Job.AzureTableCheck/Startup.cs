...
using Lykke.Job.AzureTableCheck.Modules;
using Lykke.Logs;
...
namespace Lykke.Job.AzureTableCheck
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            try
            {
                ...
                {
                    options.DefaultLykkeConfiguration("v1", "AzureTableCheck API");
                });
                ...
                builder.RegisterModule(new JobModule(appSettings.CurrentValue.AzureTableCheckJob, appSettings.Nested(x => x.AzureTableCheckJob), Log));
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, IApplicationLifetime appLifetime)
        {
            try
            {
                ...
                app.UseLykkeMiddleware("AzureTableCheck", ex => new ErrorResponse {ErrorMessage = "Technical problem"});
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, IReloadingManager<AppSettings> settings)
        {
            ...
            var dbLogConnectionStringManager = settings.Nested(x => x.AzureTableCheckJob.Db.LogsConnString);
            ...
            var persistenceManager = new LykkeLogToAzureStoragePersistenceManager(
                AzureTableStorage<LogEntity>.Create(dbLogConnectionStringManager, "AzureTableCheckLog", consoleLogger),
                consoleLogger);
